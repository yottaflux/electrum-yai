...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://github.com/nicholasgasior/yottaflux"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-yottaflux-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-yottaflux-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-yottaflux-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-yottaflux-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-yottaflux-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-yottaflux-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: and lightning: URIs to Electrum
  WriteRegStr HKCU "Software\Classes\yai" "" "URL:yai Protocol"
  WriteRegStr HKCU "Software\Classes\yai" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\yai" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\yai\shell\open\command" "" "$\"$INSTDIR\electrum-yottaflux-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  ;WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-yottaflux-${PRODUCT_VERSION}.exe$\" $\"%1$\""
